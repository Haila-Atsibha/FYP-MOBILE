...
exports.sendMessage = async (req, res) => {
    try {
        ...
        // Status check: only accepted, active, or completed bookings
        if (!['accepted', 'active', 'completed'].includes(booking.status.toLowerCase())) {
            return res.status(403).json({ message: "Chat is only available for accepted, active, or completed bookings" });
        }
        ...
        if (hasMedia) {
            ...
            const originalName = mediaFile.originalname ? mediaFile.originalname.toLowerCase() : '';
            const isImage = mimetype.startsWith('image/') || originalName.endsWith('.jpg') || originalName.endsWith('.jpeg') || originalName.endsWith('.png') || originalName.endsWith('.webp') || mimetype === 'application/octet-stream';
